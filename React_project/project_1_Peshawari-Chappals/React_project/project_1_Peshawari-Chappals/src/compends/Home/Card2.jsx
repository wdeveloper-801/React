...
const Card2 = () => {
    ...
    return (
        <>
            <div className='min-h-255 w-full'>
                ...
                <div className="card w-full h-full flex flex-wrap justify-center items-start  bg-amber-0500 gap-x-15 gap-y-15 ">
                    {data.map((item) => (
                        <div key={item.id} className='w-55 h-95 mt-2 md:max-w-50 sm:max-w-50 xl:max-w-80 lg:max-w-80 2xl:max-w-80 bg-gray-200  shadow-md hover:scale-105 transition-transform duration-300' >
                            ...
                            <div className="text h-40  flex justify-center items-center flex-col gap-2 " onClick={() => handleCardClick(item)}>
                                <p className='capitalize text-lg p-2 text-center'>{item.name}</p>
